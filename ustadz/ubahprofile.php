...
$id = $_SESSION["id_user"];
$user = query("SELECT * FROM ustadz WHERE id_ustadz = '$id'");
#var_dump ($user);
if (isset($_POST["submit"])) {
  #var_dump ($_POST);#nama, tanggal lahir, email, alamat, no telp
  #if($_POST["nama"] == ''){echo "<script>alert('nama kosong')</script>";}
  if ($_POST["nama"] == '') {
    $_POST["nama"] = $user[0]["name"];
  }
  if ($_POST["birth_date"] == '') {
    $_POST["birth_date"] = $user[0]["birth_date"];
  }
  if ($_POST["email"] == '') {
    $_POST["email"] = $user[0]["email"];
  }
  if ($_POST["address"] == '') {
    $_POST["address"] = $user[0]["address"];
  }
  if ($_POST["phone_number"] == '') {
    $_POST["phone_number"] = $user[0]["phone_number"];
  }
  if (updUstadz2($_POST, $id) > 0) {
...
<body>
  ...
  <form action="" method="post">
    <div class="picture">
      ...
      <div class="inputname">
        <input class="info name" name="nama" type="text" placeholder=<?php
    ...
    <div class="userinfo">
      <div class="contain">
        ...
        <input type="date" name="birth_date" class="info" placeholder=<?php
                                                                      if (!isset($user[0]["birth_date"])) {
                                                                        echo "";
                                                                      } else {
                                                                        echo $user[0]["birth_date"];
                                                                      }
                                                                      ?>>
        </input>
      </div>
      <div class="contain">
        ...
        <input type="email" name="email" class="info" placeholder=<?php
      ...
      <div class="contain">
        ...
        <input type="text" name="address" class="info" placeholder=<?php
      ...
      <div class="contain">
        ...
        <input type="text" name="phone_number" class="info" placeholder=<?php
